...
Security fixes are provided for the latest published version of Quota01. Before reporting a
problem, check whether it is still present in the
[latest release](https://github.com/deviffyy/OpenQuota/releases/latest).

## Credential storage

Quota01 stores provider API keys and WebView session cookies in a single encrypted credential vault.
...
Quota01-owned vault key instead of prompting separately for every provider credential.
...
- The affected Quota01 version and operating system
