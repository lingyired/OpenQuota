...
function buildWindows(target) {
  ...
  const setupExe = path.join(
    srcTauri,
    `target/${target}/release/bundle/nsis/OpenQuota_${version}_${arch}-setup.exe`,
  )
  ...
  const portableExe = path.join(srcTauri, `target/${target}/release/openquota.exe`)
  const portableName = `OpenQuota_${version}_${arch}-portable.exe`
...
function buildMacos() {
  ...
  mkdirSync(dstDir, { recursive: true })
  const copied = []
  for (const [bundle, ext] of [['dmg', '.dmg'], ['macos', '.app']]) {
