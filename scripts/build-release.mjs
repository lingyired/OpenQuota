...
function buildWindows(target) {
  ...
  const setupExe = path.join(
    srcTauri,
    `target/${target}/release/bundle/nsis/Quota01_${version}_${arch}-setup.exe`,
  );
  ...
  const portableExe = path.join(srcTauri, `target/${target}/release/quota01.exe`);
  const portableName = `Quota01_${version}_${arch}-portable.exe`;
