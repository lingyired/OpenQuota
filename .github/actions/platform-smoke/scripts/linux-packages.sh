...
extraction_directory="$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/quota01-appimage.XXXXXX")"
installed=false
cleanup() {
...
installed=true
installed_binary="$(dpkg-query --listfiles "${package_name}" | grep -E '/(usr/)?bin/quota01$' | head -n 1)"
