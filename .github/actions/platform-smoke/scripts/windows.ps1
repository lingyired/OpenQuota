...
if ($ReleaseValidation -eq 'true') {
  if ([string]::IsNullOrWhiteSpace($env:QUOTA01_EXPECTED_WINDOWS_SIGNER_SUBJECT)) {
    throw 'QUOTA01_EXPECTED_WINDOWS_SIGNER_SUBJECT is required for release validation.'
  }
...
function Assert-ReleaseSignature {
  ...
  if ($null -eq $signature.SignerCertificate -or
      $signature.SignerCertificate.Subject -ne $env:QUOTA01_EXPECTED_WINDOWS_SIGNER_SUBJECT) {
...
if (![string]::IsNullOrWhiteSpace($originalLocalAppData)) {
  $existingBinary = Join-Path $originalLocalAppData 'Quota01\Quota01.exe'
  if (Test-Path -LiteralPath $existingBinary -PathType Leaf) {
    throw "Refusing to disturb an existing Quota01 installation: $existingBinary"
  }
}
...
if (Test-Path -LiteralPath $uninstallRoot -PathType Container) {
  $existingRegistration = Get-ChildItem -LiteralPath $uninstallRoot | Where-Object {
    $_.GetValue('DisplayName') -eq 'Quota01' -or
    $_.GetValue('InstallLocation') -like '*\Quota01*' -or
    $_.GetValue('UninstallString') -like '*\Quota01\uninstall.exe*'
  } | Select-Object -First 1
  if ($null -ne $existingRegistration) {
    throw "Refusing to replace an existing Quota01 uninstall registration: $($existingRegistration.Name)"
  }
}

if (!(Test-Path -LiteralPath $InstallerDirectory -PathType Container)) {
  throw "Quota01 NSIS directory was not found: $InstallerDirectory"
}
$installers = @(Get-ChildItem -LiteralPath $InstallerDirectory -Filter '*-setup.exe' -File)
if ($installers.Count -ne 1) {
  throw "Expected exactly one Quota01 NSIS installer, found $($installers.Count)."
}
...
$smokeRoot = Join-Path $env:RUNNER_TEMP "quota01-windows-$PID"
...
$appLog = Join-Path $env:LOCALAPPDATA 'Quota01\logs\Quota01.log'
...
try {
  ...
  if ($installProcess.ExitCode -ne 0) {
    throw "Quota01 NSIS installer exited with code $($installProcess.ExitCode)."
  }

  $binaries = @(
    Get-ChildItem -LiteralPath $installRoot -Filter 'quota01.exe' -File -Recurse |
      Where-Object { $_.Name -notlike 'uninstall*' }
  )
  if ($binaries.Count -ne 1) {
    throw "Expected exactly one installed Quota01 binary, found $($binaries.Count)."
  }
  $binary = $binaries[0].FullName
  $uninstallers = @(Get-ChildItem -LiteralPath $installRoot -Filter 'uninstall*.exe' -File -Recurse)
  if ($uninstallers.Count -ne 1) {
    throw "Expected exactly one Quota01 uninstaller, found $($uninstallers.Count)."
  }
  ...
  if ($ReleaseValidation -eq 'true') {
    ...
    if ($null -eq $installerSignature.SignerCertificate -or
      ...
      throw 'The NSIS installer and installed Quota01 binary do not have the same Authenticode signer.'
    }
  }
  ...
  for ($attempt = 0; $attempt -lt 60; $attempt++) {
    if ($process.HasExited) {
      ...
      throw 'Quota01 exited during the Windows tray startup smoke test.'
    }
    if (Test-Path -LiteralPath $appLog -PathType Leaf) {
      ...
      $startupComplete = $appLogContents.Contains('Quota01 startup completed')
  ...
  if (!$trayReady -or !$startupComplete) {
    Get-Content -LiteralPath $stdout, $stderr, $appLog -ErrorAction SilentlyContinue
    throw 'Quota01 did not report a ready Windows tray before the startup deadline.'
  }
  ...
  if ($uninstallProcess.ExitCode -ne 0) {
    throw "Quota01 NSIS uninstaller exited with code $($uninstallProcess.ExitCode)."
  }
  for ($attempt = 0; $attempt -lt 30 -and (Test-Path -LiteralPath $binary); $attempt++) {
    Start-Sleep -Milliseconds 500
  }
  if (Test-Path -LiteralPath $binary) {
    throw 'Quota01 remained installed after the NSIS uninstall smoke test.'
  }
