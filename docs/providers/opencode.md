...
Quota01 combines OpenCode Go quota information with usage recorded by local OpenCode sessions.
...
Quota01 therefore shows sub-1% readings as `<1%`/`>99%` and never labels a `0` Go meter as an
unused session; the reset countdown is always shown instead.

## Sign-in and local data

Sign in to OpenCode Go or use OpenCode locally first. Quota01 reads OpenCode's local authentication
