...
Quota01 tracks Grok allowance information and usage recorded by the Grok CLI.
...
Sign in by running `grok login`. Quota01 reads the authentication data and usage history stored by
the Grok CLI. `GROK_HOME` is respected when it is set.

Spend history is estimated locally from Grok's usage log and is not uploaded by Quota01.

## Troubleshooting

- **Not logged in** — run `grok login`, then refresh Quota01.
