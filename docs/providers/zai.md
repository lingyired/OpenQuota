...
Add a Z.ai API key from **Customize** in Quota01. Saved keys are kept in the operating system's
credential store. Quota01 also checks `ZAI_API_KEY`, `GLM_API_KEY`,
`~/.config/quota01/zai.json`, and `~/.config/zai/key.json`; a key saved in the app takes priority.
