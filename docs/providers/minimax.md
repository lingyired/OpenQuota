...
then add it in **Customize** in Quota01. Saved keys are stored in the operating system's credential
store. Quota01 also checks `MINIMAX_API_KEY` and `~/.config/quota01/minimax.json`; a key saved
