...
Quota01 tracks the current DeepSeek API balance through the official balance endpoint and a
DeepSeek API key.
...
DeepSeek can return balances in more than one currency. Quota01 keeps each currency separate
...
**Customize** in Quota01. Saved keys use the encrypted credential vault protected by the operating
system credential store. Quota01 also
checks `DEEPSEEK_API_KEY` and `~/.config/quota01/deepseek.json`; a key saved in the app takes
priority.
...
Quota01 does not request or store the separate platform login token.
