...
OpenQuota01 tracks plan usage from the Cursor account already signed in on your computer.
...
Sign in through the Cursor app or run `agent login`. OpenQuota01 looks for Cursor's local application
state and platform credential storage, so no separate OpenQuota01 login is required.
...
- **History is delayed** — wait for Cursor's usage export to update, then refresh OpenQuota01.
