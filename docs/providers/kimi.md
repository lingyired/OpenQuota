...
it in **Customize** in Quota01. Saved keys are stored in the encrypted credential vault protected by
the operating system credential store. Quota01 also checks `KIMI_API_KEY` and
`~/.config/quota01/kimi.json`; a key saved in the app
takes priority.
