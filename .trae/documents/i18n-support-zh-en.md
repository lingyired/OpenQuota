...
为 Quota01 增加国际化能力，首期支持简体中文（zh-CN）与英文（en），后续可扩展更多语言。采用用户选定的 **svelte-i18n** 库；覆盖范围：前端界面全部文案、Rust 端系统通知（pacing 提醒）与 macOS 原生托盘菜单。语言偏好（跟随系统 / 英文 / 中文）作为新设置项持久化到 `AppSettings`。
...
- Rust 端用户可见文案：
  ...
  - 原生菜单：`lib.rs::install_tray()`（macOS："Settings"/"Quit Quota01"；其他平台："Open Quota01"/"Customize…"/"Settings…"/"Quit Quota01"）。`install_tray` 在 `app.manage(settings)` 之后调用，可直接读取语言偏好。
