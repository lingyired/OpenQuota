...
pub enum StorageError {
    #[error("Quota01 data directory could not be created")]
    CreateDirectory(#[source] std::io::Error),
    #[error("Quota01 database is unavailable")]
    Database(#[from] rusqlite::Error),
    #[error("Cached Quota01 data is invalid")]
    InvalidCache(#[from] serde_json::Error),
    #[error("Quota01 database lock is unavailable")]
    Poisoned,
}
...
mod tests {
    ...
    fn snapshot_round_trip_contains_no_credentials() {
        ...
        let storage = Storage::open(&directory.path().join("quota01.db")).unwrap();
        ...
        let bytes = std::fs::read(directory.path().join("quota01.db")).unwrap();
