...
const DESKTOP_FILE: &str = "com.lingyi.usage01.desktop";
...
pub fn set_enabled(enabled: bool) -> Result<(), String> {
    ...
    let executable =
        std::env::current_exe().map_err(|_| "Usage01 executable path could not be resolved.")?;
...
mod tests {
    ...
    fn honors_absolute_xdg_config_home_and_falls_back_to_home() {
        assert_eq!(
            ...
            PathBuf::from("/custom/config/autostart/com.lingyi.usage01.desktop")
        );
        assert_eq!(
            autostart_path(
            ...
            .unwrap(),
            PathBuf::from("/home/user/.config/autostart/com.lingyi.usage01.desktop")
        );
    }
    ...
    fn installs_and_removes_an_xdg_desktop_entry() {
        ...
        let path = directory.path().join("autostart/usage01.desktop");
        set_enabled_at(&path, Path::new("/opt/Open Quota/usage01"), true).unwrap();
        let entry = std::fs::read_to_string(&path).unwrap();
        assert!(entry.contains("Exec=\"/opt/Open Quota/usage01\""));
