...
//! 的实例 —— 每个 provider 一个实例：插件实例级 LeadingIcon 列图标（品牌
//! 图标）独占左侧一列，右侧上下两行渲染前两个固定指标值；创建 / 更新 /
//! 隐藏 / 移除，并监听实例点击事件（左键 → 在点击实例上方打开主窗口 popup
//! 并只聚焦该 agent；右键 → 显示隐藏 / 刷新 / 设置该 agent / 退出的原生
//! 上下文菜单）。
//!
...
pub(crate) struct TaskbandState {
    ...
    /// 实例 id -> provider id：每个实例归属同一个 provider。
    ...
    fn apply_instance(&self, app: &AppHandle, id: &str, config: AppliedConfig) {
        ...
        match created.get(id) {
            None => {
                ...
                let _ = tb.set_leading_icon(
                    id.to_string(),
                    to_icon(config.leading_icon, config.leading_icon_size),
                );
            ...
            Some(previous) => {
                ...
                if previous.leading_icon != config.leading_icon
                    || previous.leading_icon_size != config.leading_icon_size
                {
                    let _ = tb.set_leading_icon(
                        id.to_string(),
                        to_icon(config.leading_icon, config.leading_icon_size),
...
/// Windows 任务栏 LeadingIcon 的显示高度（物理像素）。
///
/// 插件在 `size: None` 时会让列图标占满整块两行高度：9pt 字号、100% 缩放
/// 下整块约 36px，比任务栏系统图标（约 24px）还大，与两行文字也不协调。
/// 这里按字号换算「两倍 em」高度（9pt @100% ≈ 24px，与系统任务栏图标
/// 相当），再乘主显示器缩放；最终由插件 clamp 到 8..=整块高度。
fn leading_icon_size_for(font_pt: f64, scale: f64) -> i32 {
    let font_pt = if font_pt.is_finite() { font_pt } else { 9.0 };
    let scale = if scale.is_finite() && scale > 0.0 {
        scale
    } else {
        1.0
    };
    (font_pt.max(1.0) * 96.0 / 72.0 * scale * 2.0)
        .round()
        .clamp(8.0, 64.0) as i32
}

#[cfg(target_os = "windows")]
fn leading_icon_size(app: &AppHandle, layout: &TaskbandLayout) -> i32 {
    let scale = app
        .primary_monitor()
        .ok()
        .flatten()
        ...
        .unwrap_or(1.0);
    leading_icon_size_for(layout.top_size.max(layout.bottom_size), scale)
}

/// 将捆绑的品牌图标 SVG 源转换为插件的 `IconSpec`。该 spec 用于实例级
/// `set_leading_icon`（LeadingIcon 列图标）：`size` 由 [`leading_icon_size`]
/// 按字号与系统缩放换算，`tint: true` 让单色图标跟随首个可见行的颜色上色
/// （缺省为品牌色 / 系统任务栏文字色）。
#[cfg(target_os = "windows")]
fn to_icon(svg: Option<&'static str>, size: i32) -> Option<IconSpec> {
...
/// 构造实例 `AppliedConfig` 所需的实例级输入（与 macOS menubar 的
/// `MenubarConfigInput` 对应），避免 `instance_config` 参数过多。
#[cfg(target_os = "windows")]
struct TaskbandConfigInput {
    side: TaskbandSide,
    order: u64,
    text: (String, String),
    lines_visible: (bool, bool),
...
/// 由实例输入 + 共有的布局配置构造一个实例的 `AppliedConfig`。
#[cfg(target_os = "windows")]
fn instance_config(
    input: TaskbandConfigInput,
    ...
    AppliedConfig {
        ...
        leading_icon_size: input.leading_icon_size,
        top_color: layout
            .top_color
            .clone()
...
pub(crate) fn update(
    ...
    for (index, provider) in settings.providers.iter().enumerate() {
        ...
        // 图标高度按字号与主显示器缩放换算，避免插件默认「占满整块」显得过大。
        let icon_size = leading_icon_size(app, &layout);
        // 每个 provider 合并为一个实例：插件实例级 LeadingIcon 列图标占据
        // 左侧独立一列（垂直居中），上下两行指标值从图标右侧开始渲染。
        // 插件同侧实例按 order 升序从边缘向里排列（Left 从 Start 向右排，
        // Right 从托盘向左排）。
        let instance_id = provider.id.clone();
...
fn context_menu_items(
    ...
    let items = vec![
        item(
            MENU_ACTION_HIDE,
            crate::i18n::bar_action_label(locale, MENU_ACTION_HIDE, provider_name),
        ),
        item(
            MENU_ACTION_REFRESH,
            crate::i18n::bar_action_label(locale, MENU_ACTION_REFRESH, provider_name),
        ),
        item(
            MENU_ACTION_SETTINGS,
            crate::i18n::bar_action_label(locale, MENU_ACTION_SETTINGS, provider_name),
        ),
        MenuItemDescriptor::Separator,
        item(
            MENU_ACTION_QUIT,
            crate::i18n::bar_action_label(locale, MENU_ACTION_QUIT, provider_name),
        ),
    ];
    let locale_code = match locale {
        crate::i18n::Locale::En => "en",
        crate::i18n::Locale::ZhCn => "zh-CN",
        crate::i18n::Locale::ZhTw => "zh-TW",
        crate::i18n::Locale::Es => "es",
        crate::i18n::Locale::PtBr => "pt-BR",
        crate::i18n::Locale::Ja => "ja",
        crate::i18n::Locale::Ko => "ko",
        crate::i18n::Locale::De => "de",
        crate::i18n::Locale::Fr => "fr",
        crate::i18n::Locale::Ru => "ru",
        crate::i18n::Locale::Hi => "hi",
        crate::i18n::Locale::Ar => "ar",
        crate::i18n::Locale::It => "it",
        crate::i18n::Locale::Pl => "pl",
        crate::i18n::Locale::Tr => "tr",
...
mod tests {
    ...
    #[test]
    fn leading_icon_size_matches_the_system_taskbar_icon_at_default_font() {
        // 9pt @100% → 2 × 12px em = 24px，与 Windows 任务栏图标一致。
        assert_eq!(leading_icon_size_for(9.0, 1.0), 24);
        // 跟随系统缩放。
        assert_eq!(leading_icon_size_for(9.0, 1.5), 36);
        assert_eq!(leading_icon_size_for(9.0, 2.0), 48);
        // 跟随字号，并始终 clamp 在插件允许的 8..=64。
        assert!(leading_icon_size_for(14.0, 1.0) > leading_icon_size_for(9.0, 1.0));
        assert_eq!(leading_icon_size_for(0.0, 0.0), 8);
        assert_eq!(leading_icon_size_for(999.0, 8.0), 64);
    }
