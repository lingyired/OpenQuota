    ...
    fn update_internal(
        ...
        if expected_account_revision != Some(self.account_revision.load(Ordering::SeqCst)) {
            let active_provider_ids = self
                .active_account_identities
                .read()
                .map_err(|_| "Quota01 account names are temporarily unavailable.".to_owned())?
                .keys()
                .cloned()
                .collect::<Vec<_>>();
        ...
        .map_err(|_| "Quota01 account names could not be saved.".to_owned())?;
        self.storage
...
mod tests {
    ...
    fn invalid_saved_settings_are_not_overwritten_with_defaults() {
        let directory = tempdir().unwrap();
        let database_path = directory.path().join("quota01.db");
