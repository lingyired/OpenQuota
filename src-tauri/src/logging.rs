...
pub fn init(path: PathBuf, level: LogLevel) {
    ...
    if let Err(error) = sink.open() {
        eprintln!("Quota01 file log disabled: {error}");
    }
...
pub fn emit(level: LogLevel, tag: &str, arguments: fmt::Arguments<'_>) {
    ...
    let Ok(mut sink) = logger.sink.lock() else {
        eprintln!("Quota01 file log disabled: lock unavailable");
        return;
    };
...
pub fn default_log_path() -> PathBuf {
    default_log_directory().join("Quota01.log")
}
...
fn default_log_directory() -> PathBuf {
    ...
    std::env::temp_dir().join("Quota01").join("logs")
}
...
mod tests {
    ...
    fn append_writes_grep_friendly_lines() {
        ...
        let path = directory.path().join("Quota01.log");
    ...
    fn default_path_uses_the_platform_log_directory_and_advertised_file_name() {
        ...
        #[cfg(target_os = "macos")]
        assert!(path.to_string_lossy().contains("Library/Logs/Quota01"));
        #[cfg(target_os = "windows")]
        assert!(path.to_string_lossy().contains("Quota01\\logs"));
        #[cfg(all(unix, not(target_os = "macos")))]
        assert!(path.to_string_lossy().contains("quota01/logs"));
    }
    ...
    fn rotation_keeps_one_archive_and_trims_oversize_on_open() {
        ...
        assert!(!directory.path().join("Quota01.2.log").exists());
    ...
    fn sink_failure_disables_file_logging_without_panicking() {
        ...
        let mut sink = LogFile::new(blocked_parent.join("Quota01.log"), 200);
